...
export default function ConfirmDialog({ open, title = 'Are you sure?', message, confirmLabel = 'Delete', danger = true, onConfirm, onCancel }) {
  if (!open) return null;
  return (
    <div className="lightbox" style={{ background: 'rgba(20,10,5,0.55)' }} onClick={onCancel}>
      ...
      >
        <AlertTriangle color={danger ? 'var(--color-danger)' : 'var(--color-vermillion)'} size={30} style={{ margin: '0 auto 10px' }} />
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button
            ...
            style={{ background: danger ? 'var(--color-danger)' : 'var(--color-vermillion)', color: '#fff' }}
            onClick={onConfirm}
          >
            {confirmLabel}
          </button>
