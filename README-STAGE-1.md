...
In `src/main.jsx`, directly **below** the existing `app.css` import:

```js
import './styles/upgrade.css';
```

That's the only change outside these files. `upgrade.css` is additive —
it adds new classes and refines a few existing ones, and deletes nothing,
so every screen you haven't swapped yet keeps rendering exactly as it
does today.
...
In `src/main.jsx`, directly **below** the `upgrade.css` line from Stage 1:

```js
import './styles/admin-premium.css';
```

`admin-premium.css` is scoped entirely to `.admin-shell` (the AdminLayout
root, now carrying that class) and `.auth-shell` (Login/Join) — it never
touches the villager-facing pages, even though it's imported globally.
...
## Two routes you'll need to add by hand

I can't edit your `App.jsx`, so wire these in yourself — both are single
lines next to the routes/imports that already exist:

**Public** — inside `Shell()`'s `<Routes>`, alongside the other
`<Route>` lines:
```jsx
...
**Admin** — inside `AdminRoutes()`, same pattern as the other
